...
def solve_assignment(
    ...
    """
    求解入口：在一个初始解上，用指定算法做改进（当前只接 ALNS）。

    关键工程约束（与你前面强调的诉求一致）：
    - 允许搜索过程短暂进入不可行，以帮助跳出局部最优；
    - 但始终维护 best_feasible（历史最优可行解）；
    - 最终优先返回 best_feasible；若全过程都没找到可行解，才退回 best（可能不可行）。

    budget：
    - time_limit_sec / max_iters 任一耗尽即停止。
    """
...
def _build_solver_diagnostics(
    *,
    ...
    return {
        ...
        "plateau_iters_after_last_improve": int(total_iters - last_best_iter) if last_best_iter is not None else int(total_iters),
...
def _solve_vnd(
    ...
    if _budget_ok(budget, t0, it):
        ...
        if improved_ev.is_feasible:
            if best_feas_ev is None or compare(improved_ev, best_feas_ev, config) < 0:
                best_feas = improved.clone(deep=True)
...
def _find_task_location(sol: AssignmentSolution, tid: int) -> Optional[Tuple[int, int]]:
    for aid, route in sol.routes.items():
        for pos, task_id in enumerate(route):
            if int(task_id) == int(tid):
