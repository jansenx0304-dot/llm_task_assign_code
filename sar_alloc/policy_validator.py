...
def validate_insertion_control(control: Any, action_space_or_candidates: Any) -> Dict[str, Any]:
    ...
    allowed_ops = _allowed(action_space_or_candidates, "allowed_insertion_operators", "insertion_operator_candidates")
    allowed_task = _allowed(action_space_or_candidates, "allowed_task_signals", "insertion_task_signal_candidates")
    allowed_pos = _allowed(action_space_or_candidates, "allowed_position_signals", "insertion_position_signal_candidates")
    validate_sparse_score_items(raw["operator_scores"], allowed_ops, field_name="insertion_control.operator_scores")
    validate_sparse_score_items(raw["task_signal_scores"], allowed_task, field_name="insertion_control.task_signal_scores")
    validate_sparse_score_items(raw["position_signal_scores"], allowed_pos, field_name="insertion_control.position_signal_scores")
    return raw


def validate_destroy_control(control: Any, action_space_or_candidates: Any) -> Dict[str, Any]:
    raw = _object(control, "destroy_control")
    _exact(raw, {"operator_scores", "signal_scores", "intensity_score"}, "destroy_control")
    allowed_ops = _allowed(action_space_or_candidates, "allowed_destroy_operators", "destroy_operator_candidates")
    allowed_signals = _allowed(action_space_or_candidates, "allowed_destroy_signals", "destroy_signal_candidates")
    validate_sparse_score_items(raw["operator_scores"], allowed_ops, field_name="destroy_control.operator_scores")
    validate_sparse_score_items(raw["signal_scores"], allowed_signals, field_name="destroy_control.signal_scores")
    _int_range(raw, "intensity_score", 0, 10, "destroy_control")
    return raw


def validate_acceptance_control(control: Any, action_space_or_candidates: Any) -> Dict[str, Any]:
    ...
    if mode not in set(_allowed(action_space_or_candidates, "allowed_acceptance_modes", "acceptance_candidates")):
...
def validate_feasibility_control(
    ...
    allowed_modes = set(candidates.names("feasibility_mode_candidates")) if candidates is not None else {"strict", "relaxed_recoverable", "recovery_only"}
    ...
    allowed_types = set(candidates.names("relaxable_violation_candidates")) if candidates is not None else set(bounds)
    seen: Set[str] = set()
    for index, value in enumerate(ratios):
        ...
        if violation_type not in allowed_types:
            raise ValueError(f"{context}.violation_type is not relaxable: {violation_type}")
        ...
        if ratio < type_bounds["min"] or ratio > type_bounds["max"]:
            raise ValueError(f"{context}.max_ratio must be in [{type_bounds['min']}, {type_bounds['max']}]")
    return raw
...
def validate_supervisor_kickoff(
    ...
    observation, candidate_set = _split_observation_candidates(observation_or_candidates, candidates)
...
def validate_supervisor_review(
    ...
    if action == "issue_contract":
        _exact(root, {"action", "contract_review", "next_contract"}, "supervisor_decision")
        ...
        return payload
    if action == "stop_run":
        _exact(root, {"action", "contract_review", "stop_explanation"}, "supervisor_decision")
        _contract_review(root["contract_review"])
...
def validate_contract(
    ...
    if observation is not None and observation.get("allowed_contract_types"):
        type_allowed &= set(str(x) for x in observation.get("allowed_contract_types", []))
    ...
    allowed_objectives = set(candidates.names("objective_candidates")) if candidates is not None else QUALITY_METRICS
    if observation is not None and observation.get("allowed_objective_metrics"):
        allowed_objectives &= set(str(x) for x in observation.get("allowed_objective_metrics", []))
    ...
    if len(set(layers)) != len(layers) or any(name not in allowed_objectives for name in layers):
        raise ValueError("contract.objective_layers must be unique allowed objective candidates")
...
def validate_solver_decision(
    ...
    *,
    ...
    observation = observation_or_candidates if isinstance(observation_or_candidates, dict) and "action_space" in observation_or_candidates else None
    candidate_source = candidates or (None if observation is not None else observation_or_candidates)
    root = _root(payload, "solver_decision")
    action = _string(root, "action", "solver_decision")
    contract_type = active_contract_type or str(((observation or {}).get("contract_view") or {}).get("contract_type", ""))
    if observation is not None:
        allowed_actions = set(str(x) for x in ((observation.get("action_space") or {}).get("allowed_actions") or []))
    ...
    if action == "construct_initial":
        ...
        validate_insertion_control(root["insertion_control"], _action_space_or_candidates(observation, candidate_source))
    elif action == "run_alns":
        allowed_fields |= {"destroy_control", "insertion_control", "acceptance_control"}
        required = {"action", "target_id", "destroy_control", "insertion_control", "acceptance_control"}
        _allowed_exact(root, allowed_fields, required, "solver_decision")
        validate_destroy_control(root["destroy_control"], _action_space_or_candidates(observation, candidate_source))
        validate_insertion_control(root["insertion_control"], _action_space_or_candidates(observation, candidate_source))
        validate_acceptance_control(root["acceptance_control"], _action_space_or_candidates(observation, candidate_source))
    else:
        _allowed_exact(root, allowed_fields, {"action", "target_id"}, "solver_decision")
    target_id = _string(root, "target_id", "solver_decision")
    if observation is not None:
        target_ids = {str(item.get("target_id", "")) for item in observation.get("decision_targets", []) if isinstance(item, dict)}
        if target_id not in target_ids:
            raise ValueError(f"solver_decision.target_id is not in decision_targets: {target_id}")
    _explanation_optional(root)
    return payload


def _global_objective(raw_value: Any, observation: Optional[Dict[str, Any]], candidates: Optional[PublicCandidates]) -> None:
    raw = _object(raw_value, "global_objective")
    _allowed_exact(raw, {"objective_layers", "explanation"}, {"objective_layers"}, "global_objective")
    layers = _string_list(raw, "objective_layers", "global_objective")
    allowed = set(candidates.names("objective_candidates")) if candidates is not None else QUALITY_METRICS
    if observation is not None and observation.get("allowed_objective_metrics"):
...
def _target_policy(raw_value: Any) -> None:
    ...
    if len(kinds) > 4 or any(kind not in TARGET_KINDS for kind in kinds):
        raise ValueError("target_policy.preferred_target_kinds contains invalid target kind")
...
def _resource_policy(raw_value: Any, remaining_budget: Optional[Dict[str, float]]) -> None:
    raw = _object(raw_value, "resource_policy")
    _exact(raw, {"min_actions", "max_actions", "max_iters", "max_time_sec"}, "resource_policy")
    ...
    if remaining_budget is not None:
        allowed_actions = min(
            int(float(remaining_budget.get("step_calls", remaining_budget.get("actions", 0)))),
            int(float(remaining_budget.get("solver_calls", remaining_budget.get("actions", 0)))),
        )
        if allowed_actions > 0 and max_actions > allowed_actions:
            raise ValueError("resource_policy.max_actions exceeds remaining action limit")
...
def _exit_conditions(raw_value: Any) -> None:
    ...
    for name in ("success", "failure"):
        ...
        if len(values) > 4:
            raise ValueError(f"exit_conditions.{name} must contain at most 4 conditions")
        seen: Set[str] = set()
        for index, value in enumerate(values):
...
def _action_space_or_candidates(observation: Optional[Dict[str, Any]], candidates: Any) -> Any:
    return (observation or {}).get("action_space") if observation is not None else candidates


def _split_observation_candidates(value: Any, candidates: Optional[PublicCandidates]) -> tuple[Optional[Dict[str, Any]], Optional[PublicCandidates]]:
    if isinstance(value, dict):
        return value, candidates
    return None, value if candidates is None else candidates


def _budget_from_observation(observation: Optional[Dict[str, Any]]) -> Optional[Dict[str, float]]:
...
def _allowed_exact(raw: Dict[str, Any], allowed: Set[str], required: Set[str], field_name: str) -> None:
    actual = set(raw)
...
def _condition_value(value: Any, field_name: str) -> None:
    ...
    if isinstance(value, (int, float)) and not isinstance(value, bool) and math.isfinite(float(value)):
        return
    raise ValueError(f"{field_name} must be a finite number, non-empty string, or boolean")
