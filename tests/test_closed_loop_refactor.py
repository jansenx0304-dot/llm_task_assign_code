...
class ClosedLoopRefactorTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.remaining = {"step_calls": 6.0, "solver_calls": 4.0, "time_sec": 5.0, "iters": 20.0}
        kickoff_obs = build_supervisor_kickoff_observation(
            instance=self.instance,
            instance_summary={"time_window_risk": {}, "energy_risk": {}, "relaxation_scale_context": {}},
            candidates=self.candidates,
            user_goal_text="cover tasks",
            remaining_global_budget=self.remaining,
            relaxation_scale_context={"time_window_median_width": 10.0, "agent_energy_median": 100.0},
        )
        raw = demo_supervisor_kickoff(kickoff_obs)["supervisor_decision"]["next_contract"]
    ...
    def test_observation_minimality(self) -> None:
        ...
        for key in ("instance_summary", "evidence_items", "memory_items", "objective_candidates", "completion_event_candidates"):
        ...
        solver = build_solver_observation(
            ...
            working_summary={"quality_summary": {"missed_priority": 2.0, "unassigned_count": 1.0}, "feasibility_summary": {"is_feasible": True, "violation_ratio_by_type": {}}},
            best_summary=None,
        ...
        for key in ("remaining_global_budget", "recent_solver_results", "evidence_items", "memory_items", "available_alns_candidates"):
            self.assertNotIn(key, solver)
        review = build_supervisor_review_observation(
            remaining_global_budget=self.remaining,
            completed_contract=self.contract.as_dict(),
            completed_contract_progress={"condition_report": [{"condition_id": "S1", "passed": True}]},
    ...
    def test_schema_outputs_validate_and_compile_to_manifest(self) -> None:
        ...
        validate_supervisor_kickoff(kickoff, kickoff_obs, self.candidates, self.config, self.remaining)
        compile_global_objective(self.config, kickoff["supervisor_decision"]["global_objective"])
        ...
        manifest = compile_solver_control(decision, self.contract, self.candidates, obs, decision_id="D1", manifest_id="R1")
    ...
    def test_runtime_manifest_alns_policy_alignment(self) -> None:
        review = demo_supervisor_review({"budget_caps": {"max_solver_actions": 3, "max_time_sec": 3.0, "max_iters": 9}})
        validate_supervisor_review(review, {"allowed_contract_types": ["alns_search"], "allowed_objective_metrics": ["missed_priority", "unassigned_count", "energy_total"]}, self.candidates, self.config, self.remaining)
        contract = compile_contract(review["supervisor_decision"]["next_contract"], "C002", self.config)
        obs = build_solver_observation(
            ...
            working_summary={"quality_summary": {"missed_priority": 2.0, "unassigned_count": 1.0, "energy_total": 4.0}, "feasibility_summary": {"is_feasible": True, "violation_ratio_by_type": {"energy": 0.0}}},
            best_summary=None,
            candidate_landscape={"candidate_stats": {}},
            recent_memory=[],
            candidates=self.candidates,
        ...
        decision = demo_solver_decision(obs)
        validate_solver_decision(decision, obs, candidates=self.candidates)
        manifest = compile_solver_control(decision, contract, self.candidates, obs, decision_id="D2", manifest_id="R2")
        policy = alns_policy_from_manifest(manifest)
        self.assertEqual(policy.destroy_policy.operator_weights["related_cluster_removal"], 8)
    ...
    def test_initial_audit_classification(self) -> None:
        class Result:
            evaluation = FakeEval({"missed_priority": 1.0, "unassigned_count": 1.0}, is_feasible=False)
            trace = {"kind": "initial_insertion", "inserted_task_count": 1, "uninserted_task_count": 1}
        ...
        class Empty:
            ...
            trace = {"kind": "initial_insertion", "inserted_task_count": 0, "uninserted_task_count": 2}
    ...
    def test_outcome_verification_and_event_taxonomy(self) -> None:
        before = FakeEval({"missed_priority": 5.0, "unassigned_count": 3.0, "energy_total": 10.0}, True)
        after = FakeEval({"missed_priority": 5.0, "unassigned_count": 3.0, "energy_total": 8.0}, True)
        trace = {
            "kind": "alns",
            "iters": 4,
            "trial_flow": {
        ...
        self.assertEqual(verification["dominant_blocker"], "feasibility_rejected_trials")
        self.assertIn("feasibility_rejected_trials", verification["event_tags"])
    ...
    def test_contract_monitor_conditions(self) -> None:
        ...
        v1 = {"verification_id": "V1", "intent_status": "not_achieved", "dominant_blocker": "feasibility_rejected_trials", "metric_delta": {"working": {"missed_priority": 0.0}}, "trace": {"trial_flow": {}, "iters": 1}}
        update_contract_progress(progress, v1)
        self.assertEqual(check_contract_completion(contract, progress, [v1])["completion_status"], "continue")
        v2 = {"verification_id": "V2", "intent_status": "not_achieved", "dominant_blocker": "feasibility_rejected_trials", "metric_delta": {"working": {"missed_priority": 0.0}}, "trace": {"trial_flow": {}, "iters": 1}}
    ...
    def test_memory_traceability_chain(self) -> None:
        memory = RunMemory()
        obs = {"observation_id": "O1", "decision_targets": [{"target_id": "T1", "kind": "energy_debt"}]}
        ...
        self.assertEqual(memory.last_verification()["verification_id"], record["verification_id"])
    ...
    def test_full_smoke_closed_loop(self) -> None:
        ...
        events = [event["event_type"] for event in solution.run_artifact["trace_events"]]
        for name in (
        ...
        for key in ("observation_id", "decision_id", "manifest_id", "trace_id", "verification_id"):
