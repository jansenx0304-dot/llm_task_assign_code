...
class T100ClosedLoopIntegrationTests(unittest.TestCase):
    def test_scripted_t100_full_loop_and_report(self) -> None:
        ...
        config = Config()
        compile_global_objective(config, {"objective_layers": ["missed_priority", "unassigned_count", "energy_total"]})
        with tempfile.TemporaryDirectory() as directory:
            result_path = Path(directory) / "result.md"
            reporter = RunReporter(result_path, {"instance": "T100", "dummy_llm": True}, use_color=False, use_emoji=False)
            solution = run_orchestrator(
                DummyLLMClient(), instance, "cover", config, Budget(time_limit_sec=1.0, max_iters=8),
                max_agent_steps=5, max_solver_calls=5, llm_mode="dummy", trace_callback=reporter.on_trace_event,
            )
            ...
            self.assertRegex(text.split("## Run Config", 1)[0], r"Status: (finished|failed)")
            ...
            for record in records:
                ...
                if keys:
                    self.assertEqual(len(keys["contract"]["key"]), len(keys["contract"]["layers"]))
                    self.assertEqual(len(keys["global"]["key"]), len(keys["global"]["layers"]))
            ...
            for event in events:
                if event["event_type"] == "compiled_contract":
                    for group in ("success", "failure"):
                        for condition in event["payload"]["exit_conditions"][group]:
                            self.assertIn(condition["source"], SUPPORTED_CONDITION_SOURCES)
                ...
                if event["event_type"] == "solver_result":
                    self.assertEqual(event["payload"]["diagnostics"]["returned_solution_source"], "action_best_feasible")
            self.assertIn("solver_requested_review", json.dumps(artifact, ensure_ascii=False))
