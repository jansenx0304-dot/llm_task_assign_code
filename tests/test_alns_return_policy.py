...
from sar_alloc.operators import (
    ...
    DESTROY_OPERATOR_NAMES, DESTROY_SIGNAL_NAMES,
    INSERTION_OPERATOR_NAMES, INSERTION_POSITION_SIGNAL_NAMES, INSERTION_TASK_SIGNAL_NAMES,
)
...
class AlnsReturnPolicyTests(unittest.TestCase):
    def test_returns_action_best_and_tracks_global_candidate(self) -> None:
        ...
        policy = CompiledALNSPolicy(
            DestroyPolicy({name: 2 for name in DESTROY_OPERATOR_NAMES}, {name: 0 for name in DESTROY_SIGNAL_NAMES}, 3, 0.14),
            InsertionPolicy({name: 2 for name in INSERTION_OPERATOR_NAMES}, {name: 0 for name in INSERTION_TASK_SIGNAL_NAMES}, {name: 0 for name in INSERTION_POSITION_SIGNAL_NAMES}),
            AcceptancePolicy("sa", 8, 0.4, 8.0),
        )
        contract_layers = [{"metric": "missed_priority", "direction": "min"}, {"metric": "energy_total", "direction": "min"}]
        global_layers = [{"metric": "energy_total", "direction": "min"}]
        result = solve_assignment(
            instance, initial, config, Budget(time_limit_sec=0.1, max_iters=8), policy,
            contract_objective_layers=contract_layers,
            global_objective_layers=global_layers,
            feasibility_policy={"mode": "strict"},
            trace_id="X1",
        )
        self.assertEqual(result.diagnostics["returned_solution_source"], "action_best_feasible")
        ...
        self.assertEqual(result.diagnostics["solution_flow"]["returned_solution_source"], "action_best_feasible")
