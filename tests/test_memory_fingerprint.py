...
class MemoryFingerprintTests(unittest.TestCase):
    def _record(self, action: str = "run_alns") -> dict:
        ...
        observation = {
            "observation_id": "O1",
            "decision_targets": [{"target_id": "T1", "kind": "unassigned_priority"}],
        }
        ...
        manifest = {
            ...
            "compiled": (
                {
                    "destroy": {
                        "operator_weights": {"random": 2, "worst": 8},
                        "signal_weights": {
                    ...
                    "insertion": {
                        "operator_weights": {"greedy": 2, "regret": 6},
                        "task_signal_weights": {
                            "priority_loss": 9,
                            "scarcity_pressure": 8,
                            "zero": 0,
                ...
                if action == "run_alns"
                else {"review_request": {"requested": True}}
            ),
        }
        ...
        return memory.record_verified_action(
            observation, decision, manifest, trace, verification
        )
    ...
    def test_fingerprint_uses_sorted_nonzero_compiled_weights(self) -> None:
        ...
        self.assertEqual(
            fingerprint["insertion_task_signal_top"],
            ["priority_loss", "scarcity_pressure"],
        )
        self.assertEqual(
            fingerprint["insertion_position_signal_top"],
            ["future_slack", "insert_cost"],
        )
        self.assertNotIn("zero", str(fingerprint))

    def test_review_does_not_fabricate_controls(self) -> None:
        self.assertEqual(
            self._record("request_supervisor_review")["control_fingerprint"],
