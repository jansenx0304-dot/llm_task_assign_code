...
class CompletionPriorityTests(unittest.TestCase):
    def _state(self) -> dict:
        return {
            "working": {
                "quality_summary": {"missed_priority": 1.0},
    ...
    def test_review_beats_conditions_and_resource(self) -> None:
        ...
        progress = ContractProgress(
            "C1", solver_actions=1, intent_status_counts={"not_achieved": 1}
        )
    ...
    def test_success_and_failure_beat_resource(self) -> None:
        ...
        self.assertEqual(
            check_contract_completion(
                success, progress, [{"action": "run_alns"}], self._state()
            )["completion_status"],
            "success",
        )
    ...
    def test_resource_when_no_review_or_condition(self) -> None:
        result = check_contract_completion(
            contract(max_actions=1),
            ContractProgress("C1", solver_actions=1),
            [{"action": "run_alns"}],
            self._state(),
        )
